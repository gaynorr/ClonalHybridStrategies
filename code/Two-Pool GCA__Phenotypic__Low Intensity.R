...
#ENDERS <- list.files("/home/mrlabroo/hybridBreeding/startEnvsTrue/testPhenoResults", full.names = TRUE)
#H2 <- c(0.0001, 0.25, 0.5, 0.75)
nCycles = 100
scenario = "GCA_PHENO1.1_maxAvoid"
...
load(STARTERS)
	...
	#ENDERJ	<- file.path(paste("/home/mrlabroo/hybridBreeding/startEnvsTrue/testPhenoResults/", scenario, "_",
     #                      "H2", 1, "_",
       ...
       #                    "_", "REP", REP,
        #                   ".RDS",
         #                  sep = ""))
			...
			#			   hold <- readRDS(ENDERJ)
			#			   
			#			   if(hold[hold$cycle == 100, "meanG.Hybrids"] != 0){
			#			   next
			#			   }
			#			   }
      ...
      for(cycle in 1:nCycles){
        ...
        if(cycle == 1){
          ...
          EliteA <- randCross(EliteA, nCrosses = 50, nProgeny = 13)
          EliteB <- randCross(EliteB, nCrosses = 50, nProgeny = 13)
        }
        ...
        if(cycle > 1){
          ...
          EliteA <- makeCross(EliteA, crossPlan = maxAvoidPlan(nInd = 100, nProgeny = 13))
          EliteB <- makeCross(EliteB, crossPlan = maxAvoidPlan(nInd = 100, nProgeny = 13))
