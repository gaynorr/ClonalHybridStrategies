...
nCycles = 100
scenario = "TWOPOOL_TBV_maxAvoidCtrlGS"
    ...
    #for(REP in 1:10){
      ...
      for(cycle in 1:nCycles){
        ...
        if(cycle == 1){
          ...
          EliteA <- randCross(EliteA, nCrosses = 50, nProgeny = 4)
          EliteB <- randCross(EliteB, nCrosses = 50, nProgeny = 4)
        }
        ...
        if(cycle > 1){
          ...
          EliteA <- makeCross(EliteA, crossPlan = maxAvoidPlan(nInd = 100, nProgeny = 4))
          EliteB <- makeCross(EliteB, crossPlan = maxAvoidPlan(nInd = 100, nProgeny = 4))
        ...
        EliteA@ebv <- bv(EliteA)
        EliteB@ebv <- bv(EliteB)
        ...
        #Elite.TrueGCA <- rbind(EliteA.TrueGCA, EliteB.TrueGCA)
        #Hybrids.TrueSCA <-  Hybrids@gv - #genetic value of the hybrid cross
        #  Elite.TrueGCA[Hybrids@mother, 1] - # mother GCA
        #  Elite.TrueGCA[Hybrids@father, 1] - # father GCA
        #  GP.Hybrids$mu # genetic mean of population
        # ploidy-agnostic
        #output[cycle, "meanTrueSCA.Hybrids"] <- mean(Hybrids.TrueSCA)
        
        
        # record the selection accuracy
        #output[cycle, "accGCA.EliteA"] <- cor(EliteA@ebv, EliteA@gv) #EliteA
        #output[cycle, "accGCA.EliteB"] <- cor(EliteB@ebv, EliteB@gv) #EliteB
        output[cycle, "acc.EliteA"] <- cor(bv(EliteA), EliteA@ebv) #EliteA
        output[cycle, "acc.EliteB"] <- cor(bv(EliteB), EliteB@ebv) #EliteB
