...
nCycles = 100
scenario = "GEBVGCA_IH_GBLUP_maxAvoidnCrosses10"
    ...
    #for(REP in 1:10){
      ...
      for(cycle in 1:nCycles){
        ...
        if(cycle == 1){
          ...
          EliteA <- randCross(EliteA, nCrosses = 10, nProgeny = 16)
          EliteB <- randCross(EliteB, nCrosses = 10, nProgeny = 16)
		  ...
		  EliteA <- selectWithinFam(EliteA, nInd = 12, use = "pheno")
		  EliteB <- selectWithinFam(EliteB, nInd = 12, use = "pheno")
        }
        ...
        if(cycle > 1){
          ...
          EliteA <- makeCross(EliteA, crossPlan = maxAvoidPlan(nInd = 20, nProgeny = 16))
          EliteB <- makeCross(EliteB, crossPlan = maxAvoidPlan(nInd = 20, nProgeny = 16))
		  ...
		  EliteA <- selectWithinFam(EliteA, nInd = 12, use = "ebv")
		  EliteB <- selectWithinFam(EliteB, nInd = 12, use = "ebv")
