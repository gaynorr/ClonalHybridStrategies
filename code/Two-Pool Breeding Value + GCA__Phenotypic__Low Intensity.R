...
nCycles = 100
scenario = "PERSEGCA_PHENO1.1_maxAvoid"
    ...
    #for(REP in 1:10){
      ...
      for(cycle in 1:nCycles){
        ...
        if(cycle == 1){
          ...
          EliteA <- randCross(EliteA, nCrosses = 50, nProgeny = 10)
          EliteB <- randCross(EliteB, nCrosses = 50, nProgeny = 10)
          
          EliteA <- setPheno(EliteA, H2 = 0.5, fixEff = cycle)
          EliteB <- setPheno(EliteB, H2 = 0.5, fixEff = cycle)
          
          EliteA <- selectWithinFam(EliteA, nInd = 8, use = "pheno")
          EliteB <- selectWithinFam(EliteB, nInd = 8, use = "pheno")
        }
        ...
        if(cycle > 1){
          ...
          EliteA <- makeCross(EliteA, crossPlan = maxAvoidPlan(nInd = 100, nProgeny = 10))
          EliteB <- makeCross(EliteB, crossPlan = maxAvoidPlan(nInd = 100, nProgeny = 10))
