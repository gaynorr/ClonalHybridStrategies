...
nCycles = 100
scenario = "PERSEGCA_PHENO1.1_maxAvoidnCrosses10"
    ...
    #for(REP in 1:10){
      ...
      for(cycle in 1:nCycles){
        ...
        if(cycle == 1){
          ...
          EliteA <- randCross(EliteA, nCrosses = 10, nProgeny = 50)
          EliteB <- randCross(EliteB, nCrosses = 10, nProgeny = 50)
          
          EliteA <- setPheno(EliteA, H2 = 0.5, fixEff = cycle)
          EliteB <- setPheno(EliteB, H2 = 0.5, fixEff = cycle)
          
          EliteA <- selectWithinFam(EliteA, nInd = 38, use = "pheno")
          EliteB <- selectWithinFam(EliteB, nInd = 38, use = "pheno")
        }
        ...
        if(cycle > 1){
          ...
          EliteA <- makeCross(EliteA, crossPlan = maxAvoidPlan(nInd = 20, nProgeny = 50))
          EliteB <- makeCross(EliteB, crossPlan = maxAvoidPlan(nInd = 20, nProgeny = 50))
