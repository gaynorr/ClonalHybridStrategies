...
nCycles = 100
scenario = "GEBVGCA_GBLUP_maxAvoidnCrosses10"
  ...
  #for(REP in 1:10){
    ...
    for(cycle in 1:nCycles){
      ...
      if(cycle == 1){
        ...
        EliteA <- randCross(EliteA, nCrosses = 10, nProgeny = 25)
        EliteB <- randCross(EliteB, nCrosses = 10, nProgeny = 25)
        
        EliteA <- setPheno(EliteA, H2= 0.5)
        EliteB <- setPheno(EliteB, H2= 0.5)
        
        EliteA <- selectWithinFam(EliteA, nInd = 20, use = "pheno")
        EliteB <- selectWithinFam(EliteB, nInd = 20, use = "pheno")
      }
      ...
      if(cycle > 1){
        ...
        EliteA <- makeCross(EliteA, crossPlan = maxAvoidPlan(nInd = 20, nProgeny = 25))
        EliteB <- makeCross(EliteB, crossPlan = maxAvoidPlan(nInd = 20, nProgeny = 25))
        ...
        EliteA <- selectWithinFam(EliteA, nInd = 20, use = "ebv")
        EliteB <- selectWithinFam(EliteB, nInd = 20, use = "ebv")
      }
      ...
      # get true GCA
      # use GPCP_polyploids.cpp (CalcCrossMeans.cpp) for ploidy-agnostic calculations
