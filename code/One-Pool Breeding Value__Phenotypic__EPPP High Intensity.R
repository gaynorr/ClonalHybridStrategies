...
nCycles = 100
scenario = "PERSE_PHENO1.1_maxAvoidnCrosses10"
    ...
    #for(REP in 1:3){
      ...
      for(cycle in 1:nCycles){
        ...
        if(cycle == 1){
          ...
          Elite <- randCross(Elite, nCrosses = 10, nProgeny = 260)
        }
        ...
        if(cycle > 1){
          ...
          Elite <- makeCross(Elite, crossPlan = maxAvoidPlan(nInd = 20, nProgeny = 260))
          
        }
        
        
        Elite <- setPheno(Elite, H2 = 0.5, fixEff = cycle)
        Elite@ebv <- Elite@pheno #for convenience
        ...
        output[cycle, "acc.Elite"] <- cor(bv(Elite), Elite@pheno)
