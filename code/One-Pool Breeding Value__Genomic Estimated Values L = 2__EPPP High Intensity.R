...
nCycles = 100
scenario = "PERSE_GEBV_maxAvoidnCrosses10"
    ...
    #for(REP in 1:3){
      ...
      for(cycle in 1:nCycles){
        ...
        if(cycle == 1){
          ...
          Elite <- randCross(Elite, nCrosses = 10, nProgeny = 130)
        }
        
        if(cycle > 1){
          
          Elite <- selectWithinFam(Elite, nInd = 2, use = "ebv") 
          
          Elite <- makeCross(Elite, crossPlan = maxAvoidPlan(nInd = 20, nProgeny = 130))
        ...
        # run the model post-flowering
        if(cycle == 1){
          Elite@ebv <- Elite@pheno # first cycle only
        }
        ...
        if(cycle > 1){
          ...
          # update trainPop for following cycle
          if(cycle == 1){
        ...
        output[cycle, "acc.Elite"] <- cor(bv(Elite), Elite@ebv)
        ...
        # record the fully selfed population value
