...
nCycles = 100
scenario = "TWOPOOL_EBV_maxAvoidnCrosses10"
    ...
    #for(REP in 1:10){
      ...
      for(cycle in 1:nCycles){
        ...
        if(cycle == 1){
          ...
          EliteA <- randCross(EliteA, nCrosses = 10, nProgeny = 20)
          EliteB <- randCross(EliteB, nCrosses = 10, nProgeny = 20)
        ...
        if(cycle > 1){
          ...
          EliteA <- makeCross(EliteA, crossPlan = maxAvoidPlan(nInd = 20, nProgeny = 20))
          EliteB <- makeCross(EliteB, crossPlan = maxAvoidPlan(nInd = 20, nProgeny = 20))
        ...
        # predict
        if(cycle == 1){
          EliteA@ebv <- EliteA@pheno
          EliteB@ebv <- EliteB@pheno
        }
        ...
        if(cycle > 1){
          ...
          #predict value
          EliteA <- setEBV(EliteA, ans, value = "bv")
          EliteB <- setEBV(EliteB, ans, value = "bv")
        }
