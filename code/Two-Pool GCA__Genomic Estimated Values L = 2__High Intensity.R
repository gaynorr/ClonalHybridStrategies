...
nCycles = 100
scenario = "GCA_GBLUP_maxAvoidnCrosses10"
...
load(STARTERS)
    ...
    #for(REP in 1:10){
      ...
      for(cycle in 1:nCycles){
        ...
        Hybrids <- c(AxB, BxA)
        Hybrids <- Hybrids[sample(c(1:nInd(Hybrids)), size = nInd(Hybrids), replace = FALSE)] #shuffle the order for the TP
        ...
        # use GPCP_polyploids.cpp (CalcCrossMeans.cpp) for ploidy-agnostic true GCA calculations
        ...
        #EliteA.TrueGCA <- as.matrix(EliteA.TrueGCA)
        #EliteB.TruceGCA <- as.matrix(EliteB.TrueGCA)
