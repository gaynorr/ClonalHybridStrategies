...
nCycles = 100
scenario = "GPCP_TRUE_maxAvoidnCrosses50CtrlGS"

nProgeny <- 26
nFamily <- 50 # same as nCrosses
  ...
  #for(j in 1){
    ...
    #for(REP in 1:10){
      ...
      for(cycle in 1:100){
        ...
        Elite <- setPheno(Elite, H2 = 1, fixEff = cycle)
        
        
        #get true cross values post-flowering
        ...
        trueCross[,4] <- paste(trueCross[,1], trueCross[,2], sep = "x")
        trueCross[,5] <- paste(trueCross[,2], trueCross[,1], sep = "x")
        
        allowedMatingsAll <- c(allowedMatings[,3], allowedMatings[,4])
        trueCrossAllowed <- trueCross[trueCross[,4] %in% allowedMatingsAll | trueCross[,5] %in% allowedMatingsAll, ]
        
        #for(i in 1:nrow(allowedMatings)){
        #  allowedMatings[i, 5] <- trueCrossAllowed[trueCrossAllowed$V4 == allowedMatings$V3[i] | trueCrossAllowed$V4 == allowedMatings$V4[i] |
        #                             trueCrossAllowed$V5 == allowedMatings$V3[i] | trueCrossAllowed$V5 == allowedMatings$V4[i], 3]
        #}
		
		
		  hex1 <- match(trueCrossAllowed$V4, allowedMatings$V3, nomatch = 0)
			allowedMatings[ , 5] <- trueCrossAllowed[hex1, 3]
        ...
        #output[cycle, "meanInbrDep.Hybrids"] <- GP.Hybrids$mu - GP.Hybrids$mu_HW #does this need an abs()? Is it backwards?
        #output[cycle, "meanInbrDep.EliteA"] <- GP.EliteA$mu - GP.EliteA$mu_HW
        #output[cycle, "meanInbrDep.EliteB"] <- GP.EliteB$mu - GP.EliteB$mu_HW
