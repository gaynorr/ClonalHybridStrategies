...
nCycles = 100
scenario = "GEBVGCA_IH_GBLUP_maxAvoid"
...
#for(i in 1:length(H2)){
  ...
  #for(j in 1:10){
	...
	#if(grepl("*_ploidy2_*", STARTERS[j]) == TRUE){
    #load(STARTERS[j])
	  load(STARTERS)
    ...
    #for(REP in 1:10){
      ...
      for(cycle in 1:nCycles){
        ...
        if(cycle == 1){
          ...
          EliteA <- randCross(EliteA, nCrosses = 40, nProgeny = 4)
          EliteB <- randCross(EliteB, nCrosses = 40, nProgeny = 4)
		  
		  EliteA <- makeDH(EliteA, nDH = 1)
		  EliteB <- makeDH(EliteB, nDH = 1)
		  
		  EliteA <- setPheno(EliteA, H2 = 0.5)
		  EliteB <- setPheno(EliteB, H2 = 0.5)
		  
		  EliteA <- selectWithinFam(EliteA, nInd = 3, use = "pheno")
		  EliteB <- selectWithinFam(EliteB, nInd = 3, use = "pheno")
        }
        
        if(cycle > 1){
          
          EliteA <- selectWithinFam(EliteA, nInd = 2, use = "ebv")
          EliteB <- selectWithinFam(EliteB, nInd = 2, use = "ebv")
          
          EliteA <- makeCross(EliteA, crossPlan = maxAvoidPlan(nInd = 80, nProgeny = 4))
          EliteB <- makeCross(EliteB, crossPlan = maxAvoidPlan(nInd = 80, nProgeny = 4))
		  ...
		  ans2 <- RRBLUP_D(trainPop, maxIter = 80)
		  EliteA <- setEBV(EliteA, ans2, value = "bv")
		  EliteB <- setEBV(EliteB, ans2, value = "bv")
          
		  EliteA <- selectWithinFam(EliteA, nInd = 3, use = "ebv")
		  EliteB <- selectWithinFam(EliteB, nInd = 3, use = "ebv")
        }
        ...
        AxB <- setPheno(AxB, H2 = 0.5, fixEff = cycle)
        BxA <- setPheno(BxA, H2 = 0.5, fixEff = cycle)
        Hybrids <- c(AxB, BxA)
        ...
        # estimate GCA post-flowering and make new within-pool crosses
        if(cycle == 1){
          aAnalysisGCA <- calcGCA(AxB, "pheno")
          bAnalysisGCA <- calcGCA(BxA, "pheno")
          EliteA@ebv <- as.matrix(aAnalysisGCA$GCAf[ ,2])
          EliteB@ebv <- as.matrix(bAnalysisGCA$GCAm[ ,2])
        }
        ...
        if(cycle > 1){
          ...
          # train the model
          ans <- RRBLUP_GCA(trainPop)
          
          #predict GCA
          EliteA <- setEBV(EliteA, ans, value = "female")
          EliteB <- setEBV(EliteB, ans, value = "male")
        }
        
        
        # update the trainPop for the following cycle
        if(cycle == 1){
          trainPop <- Hybrids
        }
        if(cycle > 1){
          trainPop <- c(trainPop, Hybrids)
        }
        ...
        # confirmed this leads to identical results as calcGCA() off a full diallel
        ...
        # used with allelic calculation for diploids only
    ...
    #} #end REP loop
    #} else { #end if ploidy == 2
	#print("Ploidy too high for makeDH")
	#}
  #}# end START loop
