...
nCycles = 100
scenario = "TWOPOOL_PHENO1.1_maxAvoidnCrosses10"
    ...
    #for(REP in 1:10){
      ...
      for(cycle in 1:nCycles){
        ...
        if(cycle == 1){
          ...
          EliteA <- randCross(EliteA, nCrosses = 10, nProgeny = 42)
          EliteB <- randCross(EliteB, nCrosses = 10, nProgeny = 42)
        ...
        if(cycle > 1){
          ...
          EliteA <- makeCross(EliteA, crossPlan = maxAvoidPlan(nInd = 20, nProgeny = 42))
          EliteB <- makeCross(EliteB, crossPlan = maxAvoidPlan(nInd = 20, nProgeny = 42))
        ...
        #output[cycle, "accGCA.EliteA"] <- cor(EliteA@ebv, bv(EliteA)) #EliteA
        #output[cycle, "accGCA.EliteB"] <- cor(EliteB@ebv, bv(EliteB)) #EliteB
        output[cycle, "acc.EliteA"] <- cor(EliteA@ebv, bv(EliteA)) #EliteA
        output[cycle, "acc.EliteB"] <- cor(EliteB@ebv, bv(EliteB)) #EliteB
