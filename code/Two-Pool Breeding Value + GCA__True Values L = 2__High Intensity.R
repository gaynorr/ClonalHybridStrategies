...
nCycles = 100
scenario = "TBVGCA_TRUE_maxAvoidnCrosses10CtrlGS"
      ...
      for(cycle in 1:nCycles){
        ...
        if(cycle == 1){
          ...
          EliteB <- randCross(EliteB, nCrosses = 10, nProgeny = 25)
		      ...
		      EliteA <- selectWithinFam(EliteA, nInd = 20, use = "bv")
		      EliteB <- selectWithinFam(EliteB, nInd = 20, use = "bv")
        }
        ...
        if(cycle > 1){
          ...
          # Select within pools on breeding value
          EliteA <- selectWithinFam(EliteA, nInd = 20, use = "bv")
          EliteB <- selectWithinFam(EliteB, nInd = 20, use = "bv")
        }
