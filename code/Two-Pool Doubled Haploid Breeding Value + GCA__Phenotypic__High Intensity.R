...
nCycles = 100
scenario = "DHPERSEGCA_PHENO1.1_maxAvoidnCrosses10"
    ...
    #for(REP in 1:10){
      ...
      for(cycle in 1:nCycles){
        ...
        if(cycle == 1){
          ...
          EliteA <- randCross(EliteA, nCrosses = 10, nProgeny = 23)
          EliteB <- randCross(EliteB, nCrosses = 10, nProgeny = 23)
          
          EliteA <- makeDH(EliteA, nDH = 1)
          EliteB <- makeDH(EliteB, nDH = 1)
          
          EliteA <- setPheno(EliteA, H2 = 0.5, fixEff = cycle)
          EliteB <- setPheno(EliteB, H2 = 0.5, fixEff = cycle)
          
          EliteA <- selectWithinFam(EliteA, nInd = 18, use = "pheno")
          EliteB <- selectWithinFam(EliteB, nInd = 18, use = "pheno")
        }
        ...
        if(cycle > 1){
          ...
          EliteA <- makeCross(EliteA, crossPlan = maxAvoidPlan(nInd = 20, nProgeny = 23))
          EliteB <- makeCross(EliteB, crossPlan = maxAvoidPlan(nInd = 20, nProgeny = 23))
